...
var configSshCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		fileBytes, err := ioutil.ReadFile(configSsh)
		if err != nil {
		...
		if !remove {
			url := fmt.Sprintf("https://development.remote.%s.libops.site/dev", site)
			...
			if resp.StatusCode > 299 {
				log.Fatalf("Unable to get development environment info: %v", resp.StatusCode)
			}
...
# to make accessing your LibOps development environment easier.
#
...
# You should not hand-edit this section, unless you are deleting it.`,
		...
		if match == nil {
			...
			err := ioutil.WriteFile(configSsh, append(fileBytes, newBlock...), 0644)
			if err != nil {
			...
			err = ioutil.WriteFile(configSsh, newFileBytes, 0644)
			if err != nil {
				log.Printf("Failed to update %s", configSsh)
				log.Fatal(err)
			}
